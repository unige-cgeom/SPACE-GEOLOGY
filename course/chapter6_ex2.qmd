...
pixel_count = np.sum(rasterized == 1)
...
dx = (north_east + 2*east + south_east - north_west - 2*west - south_west) / (8 * pixel_width)
dy = (south_west + 2*south + south_east - north_west - 2*north - north_east) / (8 * pixel_height)
...
```{python}
print(f"Min slope:  {np.nanmin(slope_deg):.2f}°")
print(f"Max slope:  {np.nanmax(slope_deg):.2f}°")
...
plt.imshow(slope_deg, cmap="magma", extent=[left, right, bottom, top])
plt.colorbar(label="Slope (degrees)")
plt.show()

with rasterio.open(
    "data/outputs/slope.tif", "w",
    driver="GTiff",
    height=shape[0], width=shape[1],
...
1.4. Sampling in a sub-equilibrium zone
